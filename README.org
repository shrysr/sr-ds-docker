...
** YAML for CI
:PROPERTIES:
:header-args: :mkdirp yes :tangle ./rbase/build-all.yml
...
jobs:
  ...
  build:
    ...
    - name: Build rbase
      run: docker build . --file rbase/Dockerfile --tag my-image-name:$(date +%s)
...
jobs:
  ...
  build:
    ...
    - uses: actions/checkout@v1
    - name: Build asmith
      run: docker build . --file asmith/Dockerfile --tag my-image-name:$(date +%s)
    - name: Build rbase
      run: docker build . --file rbase/Dockerfile --tag my-image-name:$(date +%s)
    - name: Build rstudio
      run: docker build . --file rstudio/Dockerfile --tag my-image-name:$(date +%s)
    - name: Build shiny
      run: docker build . --file shiny/Dockerfile --tag my-image-name:$(date +%s)
#+END_SRC
* TODO Rstudio
:PROPERTIES:
:header-args: :mkdirp yes
